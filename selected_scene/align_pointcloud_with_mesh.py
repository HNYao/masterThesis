...
npy_file = np.load("selected_scene/align_pointcloud_with_mesh.npy", allow_pickle=True).item()
...
scene_rotation_matrix = np.array(npy_file["scene_rotation_matrix"])
...
#print(T_plane_without_obj)

o3d.visualization.draw_geometries([scene_comp_pcd, pts_without_obj, obj_pcd])

to_save = {
    "obj_scale": obj_scale,
    "scene_mesh_point": np.asarray(scene_comp_pcd.points), # point from mesh
    "scene_pcd_point":np.asarray(pts_without_obj.points), # point from rgbd
