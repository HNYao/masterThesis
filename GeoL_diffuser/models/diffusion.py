...
class SinusodialPosEmb(nn.Module):
    def __init__(self, dim):
        super(SinusodialPosEmb, self).__init__()
        self.dim = dim
    ...
    def init_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Linear):
                nn.init.kaiming_normal_(m.weight) # or xavier_normal
...
class Diffusion(nn.Module):
    def __init__(self, loss_type, beta_schedule="linear", clip_denoised=True, predict_epsilon=True ,**kwargs): 
        ...
        self.T = kwargs["T"] # backward steps
        ...
        self.register_buffer("posterior_log_variance_clipped", torch.log(posterior_variance.clamp(min=1e-20)))
            # x_t is knew, calculate x_0 q(x_{t-1 | x_t, x_0})})
    ...
    def sample(self, state, *args, **kwargs):
        ...
        return action.clamp(-1, 1) # NOTE: clamp_(-1, 1)
    ...
    def p_mean_variance(self, x, t, s):
        ...
        x_recon = self.predict_start_from_noise(x, t, pred_noise) # x_0
    ...
    def p_sample_loop(self, state, shape, *args, **kwargs):
        device = self.device
        batch_size = state.shape[0]
        x = torch.randn(shape, device=device, requires_grad=True) # noise

        # reverse
        for i in reversed(range(0, self.T)):
            t = torch.full((batch_size, ), i, device=device, dtype=torch.long) # time step
    ...
    def p_losses(self, x_start, state, t, weights=1.0):
        ...
        x_recon = self.predict_start_from_noise(x_noisy, t, noise)
