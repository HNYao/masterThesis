...
GPT_COLOR = [0.190, 0.627, 0.677] # blue
LLAVA05_COLOR = [0.741, 0.851, 0.133]
LLAVA7_COLOR = [0.398, 0.682, 0.168]
ROBOPOINT_COLOR = [0.937, 0.313, 0.576]
CLIP_COLOR = [0.855, 0.431, 0.090]
...
def visualize_npz(args):
    ...
    project_fpath = os.path.join(render_dir, "project_baselines.blend")
    if args.overwrite_project and os.path.exists(project_fpath):
        os.remove(project_fpath)
    render_fpath = os.path.join(render_dir, "render_baselines.png")
    ...
    import pdb
    pdb.set_trace()
    # check if there is pred_xyz_chat
    if "pred_xyz_all_gpt" in data.keys():
        pred_xyz_all_gpt = data["pred_xyz_all_gpt"]
    if "pred_xyz_all_llava05" in data.keys():
        pred_xyz_all_llava05 = data["pred_xyz_all_llava05"]
    if "pred_xyz_all_llava7" in data.keys():
        pred_xyz_all_llava7 = data["pred_xyz_all_llava7"]
    if "pred_xyz_all_robopoint" in data.keys():
        pred_xyz_all_robopoint = data["pred_xyz_all_robopoint"]
    if "pred_xyz_all_clip" in data.keys():
        pred_xyz_all_clip = data["pred_xyz_all_clip"]
    ...
    obj_mesh_file = "data_and_weights/mesh_realworld/cake.obj"
    ...
    if "pred_xyz_all_gpt" in data.keys():
        print("pred_xyz_all_gpt:", pred_xyz_all_gpt)
        for i in range(len(pred_xyz_all_gpt)):
            ...
            obj_mesh_i.paint_uniform_color(GPT_COLOR)
            dR_object = SciR.from_euler("Z", 0, degrees=True).as_matrix()
            xyz_object = pred_xyz_all_gpt[i]
            pred_xyz_all_gpt[i] = xyz_object
    ...
    if "pred_xyz_all_llava05" in data.keys():
        print("pred_xyz_all_llava05:", pred_xyz_all_llava05)
        for i in range(len(pred_xyz_all_llava05)):
            # guide_loss_color_i = guide_loss_color[i]
            # # Create a mesh by copying the vertices and faces of the original mesh
            obj_mesh_i = o3d.geometry.TriangleMesh()
            obj_mesh_i.vertices = obj_mesh.vertices
            ...
            obj_mesh_i.paint_uniform_color(LLAVA05_COLOR)
            dR_object = SciR.from_euler("Z", 0, degrees=True).as_matrix()
            xyz_object = pred_xyz_all_llava05[i] 
            pred_xyz_all_llava05[i] = xyz_object
    ...
    if "pred_xyz_all_llava7" in data.keys():
        print("pred_xyz_all_llava7:", pred_xyz_all_llava7)
        for i in range(len(pred_xyz_all_llava7)):
            # guide_loss_color_i = guide_loss_color[i]
            # # Create a mesh by copying the vertices and faces of the original mesh
            obj_mesh_i = o3d.geometry.TriangleMesh()
            obj_mesh_i.vertices = obj_mesh.vertices
            ...
            obj_mesh_i.paint_uniform_color(LLAVA7_COLOR)
            dR_object = SciR.from_euler("Z", 0, degrees=True).as_matrix()
            xyz_object = pred_xyz_all_llava7[i] 
            pred_xyz_all_llava7[i] = xyz_object
    ...
    if "pred_xyz_all_robopoint" in data.keys():
        print("pred_xyz_all_robopoint:", pred_xyz_all_robopoint)
        for i in range(len(pred_xyz_all_robopoint)):
            obj_mesh_i = o3d.geometry.TriangleMesh()
            obj_mesh_i.vertices = obj_mesh.vertices
            obj_mesh_i.triangles = obj_mesh.triangles
            obj_mesh_i.compute_vertex_normals()
            obj_mesh_i.paint_uniform_color(ROBOPOINT_COLOR)
            dR_object = SciR.from_euler("Z", 0, degrees=True).as_matrix()
            xyz_object = pred_xyz_all_robopoint[i] 
            pred_xyz_all_robopoint[i] = xyz_object
            obj_mesh_i.rotate(dR_object, center=[0, 0, 0])
            obj_mesh_i.rotate(T_plane[:3, :3], center=[0, 0, 0])
            obj_mesh_i.translate(xyz_object)  # move obj
            vis_o3d.append(obj_mesh_i)
    ...
    if "pred_xyz_all_clip" in data.keys():
        print("pred_xyz_all_clip:", pred_xyz_all_clip)
        for i in range(len(pred_xyz_all_clip)):
            obj_mesh_i = o3d.geometry.TriangleMesh()
            obj_mesh_i.vertices = obj_mesh.vertices
            obj_mesh_i.triangles = obj_mesh.triangles
            obj_mesh_i.compute_vertex_normals()
            obj_mesh_i.paint_uniform_color(CLIP_COLOR)
            dR_object = SciR.from_euler("Z", 0, degrees=True).as_matrix()
            xyz_object = pred_xyz_all_clip[i] 
            pred_xyz_all_clip[i] = xyz_object
            obj_mesh_i.rotate(dR_object, center=[0, 0, 0])
            obj_mesh_i.rotate(T_plane[:3, :3], center=[0, 0, 0])
            obj_mesh_i.translate(xyz_object)
            vis_o3d.append(obj_mesh_i)
    ...
    if args.blender_render == True:
        ...
        if "pred_xyz_all_gpt" in data.keys():
            ...
            for i in range(len(pred_xyz_all_gpt)):
                ...
                obj_mesh_i.rotate(dR_object, center=[0, 0, 0])
                obj_mesh_i.rotate(T_plane[:3, :3], center=[0, 0, 0])  # rotate obj mesh
                obj_mesh_i.translate(pred_xyz_all_gpt[i])  # move obj
        ...
        if "pred_xyz_all_llava05" in data.keys():
            ...
            for i in range(len(pred_xyz_all_llava05)):
                ...
                obj_mesh_i.rotate(dR_object, center=[0, 0, 0])
                obj_mesh_i.rotate(T_plane[:3, :3], center=[0, 0, 0])
                obj_mesh_i.translate(pred_xyz_all_llava05[i])  # move obj
                o3d.io.write_triangle_mesh("visual_tool/obj.ply", obj_mesh_i)  # Save the mesh ..
                mesh = bt.readMesh("visual_tool/obj.ply", (0, 0, 0), (0, 0, 0), (1, 1, 1))
                bpy.ops.object.shade_smooth()
                color = list(LLAVA05_COLOR) + [0.0] 
                meshColor = bt.colorObj(color, 0.5, 1.0, 1.0, 0.0, 0.0)
                AOStrength = 0.5
                metalVal = 0.9
                bt.setMat_metal(mesh, meshColor, AOStrength, metalVal)
        ...
        if "pred_xyz_all_llava7"  in data.keys():
            ...
            for i in range(len(pred_xyz_all_llava7)):
                ...
                obj_mesh_i.rotate(dR_object, center=[0, 0, 0])
                obj_mesh_i.rotate(T_plane[:3, :3], center=[0, 0, 0])
                obj_mesh_i.translate(pred_xyz_all_llava7[i])
                o3d.io.write_triangle_mesh("visual_tool/obj.ply", obj_mesh_i)  # Save the mesh ..
                mesh = bt.readMesh("visual_tool/obj.ply", (0, 0, 0), (0, 0, 0), (1, 1, 1))
                bpy.ops.object.shade_smooth()
                color = list(LLAVA7_COLOR) + [0.0]
                meshColor = bt.colorObj(color, 0.5, 1.0, 1.0, 0.0, 0.0)
                AOStrength = 0.5
                metalVal = 0.9
                bt.setMat_metal(mesh, meshColor, AOStrength, metalVal)
        ...
        if "pred_xyz_all_robopoint"  in data.keys():
            ...
            for i in range(len(pred_xyz_all_robopoint)):
                ...
                obj_mesh_i.rotate(dR_object, center=[0, 0, 0])
                obj_mesh_i.rotate(T_plane[:3, :3], center=[0, 0, 0])
                obj_mesh_i.translate(pred_xyz_all_robopoint[i])
                o3d.io.write_triangle_mesh("visual_tool/obj.ply", obj_mesh_i)  # Save the mesh ..
                mesh = bt.readMesh("visual_tool/obj.ply", (0, 0, 0), (0, 0, 0), (1, 1, 1))
                bpy.ops.object.shade_smooth()
                color = list(ROBOPOINT_COLOR) + [0.0]
                meshColor = bt.colorObj(color, 0.5, 1.0, 1.0, 0.0, 0.0)
                AOStrength = 0.5
                metalVal = 0.9
                bt.setMat_metal(mesh, meshColor, AOStrength, metalVal)
        ...
        if "pred_xyz_all_clip" in data.keys():
            ...
            for i in range(len(pred_xyz_all_clip)):
                ...
                obj_mesh_i.rotate(dR_object, center=[0, 0, 0])
                obj_mesh_i.rotate(T_plane[:3, :3], center=[0, 0, 0])
                obj_mesh_i.translate(pred_xyz_all_clip[i])
                o3d.io.write_triangle_mesh("visual_tool/obj.ply", obj_mesh_i)
                mesh = bt.readMesh("visual_tool/obj.ply", (0, 0, 0), (0, 0, 0), (1, 1, 1))
                bpy.ops.object.shade_smooth()
                color = list(CLIP_COLOR) + [0.0]
                meshColor = bt.colorObj(color, 0.5, 1.0, 1.0, 0.0, 0.0)
                AOStrength = 0.5
                metalVal = 0.9
                bt.setMat_metal(mesh, meshColor, AOStrength, metalVal)
                break
            if len(pred_xyz_all_clip) == 0:
                obj_mesh_i = o3d.geometry.TriangleMesh()
                obj_mesh_i.vertices = obj_mesh.vertices
                obj_mesh_i.triangles = obj_mesh.triangles
                obj_mesh_i.compute_vertex_normals()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--npz_path",
        type=str,
        default="qualitative_demo/qualitative_npz/cake_plate_rw.npz",
    )
    parser.add_argument(
        "--obj_mesh",
        type=str,
