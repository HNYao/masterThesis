    ...
    for data_batch in dataloader:
        ...
        for i, batch in enumerate(dataloader_one_case):
            ...
            with torch.no_grad():
                ...
                pred = model_diffusion(batch, num_samp=1, class_free_guide_w=-0.5, apply_guidance=False, guide_clean=True)  
                ...
                guide_affordance_loss = pred["guide_losses"]["affordance_loss"][0].cpu().numpy()
                guide_collision_loss = pred["guide_losses"]["collision_loss"][0].cpu().numpy()
                guide_distance_error = pred["guide_losses"]["distance_error"][0].cpu().numpy()
                ...
                # non collision rate
                non_collision = guide_collision_loss < 1e-6
                non_collision_result += non_collision.tolist()
                print("current non_collision rate:", sum(non_collision_result) / len(non_collision_result) * 100, "%") 

                # mean distance error
                mean_distance.append(guide_distance_error.mean())
                print("current mean distance error:", np.mean(mean_distance))

                # min distance error
                min_distance.append(guide_distance_error.min())
                print("current min distance error:", np.mean(min_distance))

            break
    
    print("Final non_collision rate:", sum(non_collision_result) / len(non_collision_result) * 100, "%")
    print("Final mean distance error:", np.mean(mean_distance))
    print("Final min distance error:", np.mean(min_distance))
