...
class GeoL_net(nn.Module):
    def __init__(self, input_shape, target_input_shape, intrinsics=None):
        ...
        if intrinsics is None:
            self.intrinsics = np.array([[591.0125 ,   0.     , 322.525  ],
    ...
    def forward(self, **kwargs):
        ...
        # merge
        x_align = self.concate(x_rgb["affordance"], scene_pcs, self.intrinsics).permute(0,2,1) # [B, Num_pts, C_rgbfeat]
...
class GeoL_net_v3(nn.Module):
    ...
    def __init__(self, input_shape, target_input_shape):
        ...
        self.feat_perceiver = FeaturePerceiver(transition_dim=115,
                                               condition_dim=256,
                                            #    time_emb_dim=64
                                               )
    ...
    def forward(self, **kwargs):
        ...
        # 处理 direction text
        x_anchor_and_text = torch.cat((x_anchor_position, d_input), dim=1) # [B,  (256 + 256) ]
...
class GeoL_net_v4(nn.Module):
    ...
    def forward(self, **kwargs):
        ...
        #encode text(direction)
        direction_inputs_indices = torch.tensor(list(map(self.vocab.get, batch['direction_text'])))
        d_input = torch.eye(len(self.vocab))[direction_inputs_indices].cuda() # [batch_size, 8] one-hot
        d_input = self.direction_mlp(d_input) # direction: [B, 256]
        # point cloud
        scene_pcs = batch["fps_points_scene"]
        obj_pcs = batch["fps_points_scene"]
        ...
        # 处理 direction text
        x_anchor_and_text = torch.cat((x_anchor_position, d_input), dim=1) # [B,  (128 + 256) ]
...
class GeoL_net_v5(nn.Module):
    ...
    def __init__(self, input_shape, target_input_shape):
        ...
        self.feat_perceiver = FeaturePerceiver(transition_dim=3,
                                               condition_dim=8,
                                               time_emb_dim=2
                                               )
...
class GeoL_net_v6(nn.Module):
    ...
    def __init__(self, input_shape, target_input_shape):
        ...
        self.direction_mlp = nn.Sequential(
            nn.Linear(8,256),
            nn.TransformerDecoderLayer(d_model=256, nhead=4, dim_feedforward=512, batch_first=True)
        )
        ...
        self.feat_perceiver = FeaturePerceiver(transition_dim=118,
                                               condition_dim=256,
                                               time_emb_dim=32
                                               )
    ...
    def update_text(self, direction_text_batch):
        ...
        for i, text in enumerate(direction_text_batch):
            if text == "Left":
                direction_text_batch[i] = "Left, After a long and exhausting day at work, I left the office feeling both relieved and tired, looking forward to finally going home, where I could rest and unwind peacefully\
                    The left part of my car is damaged. I need to take it to the repair shop to get it fixed."
            elif text == "Right":
                direction_text_batch[i] = "Right, The right side of the brain is responsible for creativity. do you like to draw or paint? If so, you are using the right side of your brain\
                ...
                direction_text_batch[i] = "Front is the best. The front of the house is where the garden is located. The garden is a beautiful place to relax and enjoy the outdoors.\
                    TUM is a university in Munich, Germany. It is located in the front of the city, near the city center. "
            elif text == "Behind":
                direction_text_batch[i] = "Behind hahaha The cat is hiding behind the couch. I can see its tail sticking out from behind the couch.\
    ...
    def forward(self, **kwargs):
        ...
        # encode text(object name)
        l_enc, l_emb, l_mask = self.encode_text(direction_text) # encode the direction text
        l_input = l_emb if 'word' in self.lang_fusion_type else l_enc
        l_input = l_input.to(dtype=torch.float32)
        batch["target_query"] = l_input

        d_enc, d_emb, d_mask = self.direction_encoder(direction_text) # encode the direction text
        ...
        # merge 得到场景特征 
        x_align = self.concate(x_rgb["affordance"], scene_pcs, self.instrics).permute(0,2,1) # [B, 64, 2048]
        x_scene = torch.cat((scene_pcs.permute(0,2,1), obj_pcs.permute(0,2,1), x_align, x_geo),dim=1) # 场景特征 [B, 118, 2048]
        # each position minues anchor position
        ...
        # 处理 direction text
        x_anchor_and_text = torch.cat((x_anchor_position, d_input), dim=1) # [B,  (256 + 1024) ]
...
class GeoL_net_v7(nn.Module):
    ...
    def __init__(self, input_shape, target_input_shape):
        super().__init__()
        self.ContactMLP = ContactMLP(contact_dim=64, point_feat_dim=64, text_feat_dim=256)
        self.clipunet_module = CLIPUNet(input_shape=input_shape, target_input_shape=target_input_shape, output_dim=64) #6kw parameters
        self.concate = FeatureConcat()
        self.device = "cuda" # cpu for dataset
        self.lang_fusion_type = 'mult' # hard code from CLIPlingunet
        ...
        self.feat_perceiver = FeaturePerceiver(transition_dim=134,
                                               condition_dim=256,
                                               time_emb_dim=32
                                               )
    ...
    def forward(self, **kwargs):
        ...
        dist_field = self.calculate_distance_field(batch["fps_points_scene"], anchor_pos) # [B, Num_points, 1]
        dist_field = self.dist_mlp(dist_field) # [B, Num_points, 64]
        
        output = {}

        # encode text(object name)
        l_enc, l_emb, l_mask = self.encode_text(direction_text) #use direction input as target query
        l_input = l_emb if 'word' in self.lang_fusion_type else l_enc
        l_input = l_input.to(dtype=torch.float32)
        ...
        # point cloud
        scene_pcs = batch["fps_points_scene"]
        obj_pcs = batch["fps_points_scene"] - anchor_pos.unsqueeze(1) # [B, Num_points, 3]

        obj_pcs_feat = self.obj_mlp(obj_pcs) # [B, Num_points, 64]
        x_geo = self.ContactMLP(dist_field, obj_pcs_feat, d_input.unsqueeze(1)) # [B, Num_points, 64]

        # CLIP: extract the rgb feat.
        x_rgb = self.clipunet_module(batch=batch) # [B, C_rgbfeat, H, W] [B, 64, H, W]

        # merge 得到场景特征 
        x_align = self.concate(x_rgb["affordance"], scene_pcs, self.instrics).permute(0,2,1) # [B, 64, num_points]
        x_scene = torch.cat((scene_pcs, obj_pcs, x_align.permute(0,2,1), x_geo),dim=2) # 场景特征 [B, num_points, 138]
        # each position minues anchor position
        #x_scene = x_scene.permute(0,2,1) # [B, 2048, 118]    
...
class GeoL_net_v8(nn.Module):
    ...
    def __init__(self, input_shape, target_input_shape):
        ...
        self.feat_perceiver_2 = FeaturePerceiver(transition_dim=256,
                                               condition_dim=256,
                                               time_emb_dim=32
                                               )
    ...
    def update_text(self, direction_text_batch):
        ...
        for i, text in enumerate(direction_text_batch):
            if text == "Left":
                ...
                direction_text_batch[i] = "Left Front yes it is. The Left Front is a political party in the United States. It was founded in 2004 by a group of former members of the Democratic Party."
            elif text == "Right Front":
                direction_text_batch[i] = "Right Front well done. The Right Front china is a desktop publishing software application developed by Adobe Systems. It is used to create documents, such as newsletters, brochures, and flyers."
            elif text == "Left Behind":
                direction_text_batch[i] = "Left Behind is a novel by Tim LaHaye and Jerry B. Jenkins that was first published in 1995. It is the first book in the Left Behind series, which has sold over 65 million copies worldwide."
            elif text == "Right Behind":
                direction_text_batch[i] = "Right Behind is a song by the American rock band Nine Inch Nails. It was released as the third single from their second studio album, The Downward Spiral (1994)."
        return direction_text_batch
    ...
    def forward(self, **kwargs):
        ...
        # encode text(object name)
        l_enc, l_emb, l_mask = self.encode_text(reference_obj_name) # encode the direction text
        l_input = l_emb if 'word' in self.lang_fusion_type else l_enc
        l_input = l_input.to(dtype=torch.float32)
        batch["target_query"] = l_input
...
class GeoL_net_v8(nn.Module):
    ...
    def __init__(self, input_shape, target_input_shape, intrinsics=None):
        ...
        self._load_clip()
        if intrinsics is None:
            self.intrinsics = np.array([[607.09912/2 ,   0.     , 636.85083/2  ],
    ...
    def forward(self, **kwargs):
        ...
        # merge 得到场景特征 
        x_align = self.concate(x_rgb["affordance"], scene_pcs, self.intrinsics).permute(0,2,1) # [B, 64, 2048]
        x_scene = torch.cat((scene_pcs.permute(0,2,1), obj_pcs.permute(0,2,1), x_align, x_geo),dim=1) # 场景特征 [B, 118, 2048]
        # each position minues anchor position
        x_scene = x_scene.permute(0,2,1) # [B, 2048, 118]    
...
if __name__ == "__main__":
    ...
    batch["file_path"] = ["dataset/scene_RGBD_mask/id162_1/lamp_0004_orange/pc_ori.ply", "dataset/scene_RGBD_mask/id162_1/lamp_0004_orange/pc_ori.ply"]
