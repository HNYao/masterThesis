...
from metrics.dataset_factory import BlendprocDesktopDataset
from Geo_comb.pred_one_case_completed import generate_heatmap_pc
from Geo_comb.pred_one_case_diffuser_dryrun import pred_one_case_dataset
...
def GeoL_completed_metrics(
    ...
    state_affordance_dict = torch.load(
        pretrained_affordance, map_location="cpu"
    )
    model_affordance.load_state_dict(state_affordance_dict["ckpt_dict"])

    state_diffusion_dict = torch.load(pretrained_diffusion, map_location="cpu") # test
    ...
    for data_batch in dataloader:
        ...
        for i, batch in enumerate(dataloader_one_case):
            ...
            with torch.no_grad():
                ...
                print("min affordance loss:", guide_affordance_loss[0][min_guide_loss_idx])
                ...
                # #guide_composite_loss[guide_collision_loss > 0.0001] = np.inf
                # min_guide_loss_idx = np.argsort(guide_composite_loss)[0][:topk]
                # pred_points = pred_points[min_guide_loss_idx]

                # print("min affordance loss:", guide_composite_loss[0][min_guide_loss_idx])
                ...
                print("current:", sum(total_result) / len(total_result))

            break

    total_result = sum(total_result) / len(total_result)
    return total_result
...
if __name__ == "__main__":
    print(len(BlendprocDesktopDataset()))
    GeoL_completed_metrics(
        dataset=BlendprocDesktopDataset(),
        sample_datasize=100,
        pretrained_affordance="outputs/checkpoints/GeoL_v9_10K_meter_retrain/ckpt_29.pth",
        pretrained_diffusion="outputs/checkpoints/GeoL_diffuser_v0__topk_1K/ckpt_21.pth",
        process_metric_func=process_direction_metrics_GeoL_completed,
    )
    # outputs/checkpoints/GeoL_diffuser_v0__topk_1K/ckpt_21.pth